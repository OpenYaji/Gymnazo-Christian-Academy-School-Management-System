...
export default function DashboardSidebar({ collapsed, setCollapsed, mobileOpen, setMobileOpen }) {
  ...
  const menuItems = [
    ...
    { name: 'Academic', icon: <GraduationCap size={20} />, path: '/student-dashboard/academic' },
    { name: 'Transaction', icon: <CreditCard size={20} />, path: '/student-dashboard/transaction' },
    { name: 'Settings', icon: <Settings size={20} />, path: '/student-dashboard/settings' },
  ];
  ...
  const NavLink = ({ item, isActive, isCollapsed = false, isMobileLink = false }) => (
    <li className="relative group">
      <Link
        ...
        className={`flex items-center gap-4 p-3 rounded-xl transition-colors ${isActive && item.name !== 'Logout' ? "bg-[#F3D67D] text-black font-bold" : "hover:bg-white/10"

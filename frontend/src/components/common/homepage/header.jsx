...
const Header = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Check if scrolled away from home section
  ...
  const headerClasses = isScrolledFromHome
    ...
    : 'bg-black/5 dark:bg-gray-900/5 backdrop-blur-2xl';

  const navClasses = isScrolledFromHome
    ? 'bg-gray-50 dark:bg-gray-800'
    : 'bg-white/10 dark:bg-gray-700/10 backdrop-blur-md';

  // Dynamic text color classes
  const textColorClasses = isScrolledFromHome
    ? 'text-[#5B3E31] dark:text-amber-400'
    : 'text-[#F4D77D] dark:text-amber-400';

  // Dynamic logo border color
  const logoBorderClasses = isScrolledFromHome
    ? 'border-[#5B3E31] dark:border-amber-400'
    : 'border-[#F4D77D] dark:border-amber-400';

  // Dynamic login button classes
  const loginButtonClasses = isScrolledFromHome
    ? 'bg-[#F4D77D] hover:bg-[#F7C236] dark:bg-amber-500/20 dark:hover:bg-amber-400/30 border-[#5B3E31] dark:border-amber-400'
    : 'bg-white/20 hover:bg-white/30 dark:bg-amber-500/20 dark:hover:bg-amber-400/30 border-white/30 dark:border-amber-400';

  return (
    <header className={`w-full shadow-lg sticky top-0 z-50 border-b border-white/10 dark:border-gray-700/10 transition-all duration-500 ${headerClasses}`}>
          ...
          <div className='flex items-center'>
            ...
            <div className='flex items-center gap-2 sm:gap-4'>
                <button
                    ...
                    className="relative w-8 h-8 rounded-full bg-white/20 dark:bg-gray-700/20 backdrop-blur-sm border border-white/30 dark:border-amber-400 flex items-center justify-center transition-all duration-300 hover:scale-110 shadow-sm"
                ...
                <button className={`backdrop-blur-sm ${textColorClasses} lg:mr-10 font-bold py-1.5 px-3 sm:py-2 sm:px-4 border-2 rounded-full shadow-lg text-xs sm:text-base transition-all duration-300 ${loginButtonClasses}`}>
      ...
      <nav className={`hidden lg:flex justify-center ${textColorClasses} py-3 text-sm font-bold transition-all duration-500 border-t border-white/10 dark:border-gray-700/10 ${navClasses}`}>
                ...
                <a
                    ...
                    className={`
                        ...
                        ${activeNavIndex === index 
                            ? 'text-[#5B3E31] dark:text-amber-400 bg-white/20 dark:bg-amber-400/20 backdrop-blur-sm' 

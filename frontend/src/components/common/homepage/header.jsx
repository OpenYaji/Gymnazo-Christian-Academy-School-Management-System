...
const Header = () => {
  ...
  // Dark mode effect - no changes needed here
  useEffect(() => {
    if (isDarkMode) {
  ...
  // Scroll spy effect - logic is correct
  useEffect(() => {
    const handleScroll = () => {
      // Defines the IDs of the sections to track
      const sections = ['home', 'about-us', 'announcement', 'contact-us'];
      // Adds a 150px offset to trigger the change earlier
  ...
  return (
    <header className='w-full shadow-md sticky top-0 z-50'>
      ...
      <nav className='hidden lg:flex justify-center bg-[#F4D77D] dark:bg-gray-700 text-[#5B3E31] dark:text-amber-400 py-3 text-sm font-bold transition-colors duration-300'>
                ...
                <a
                    ...
                    className={`
                        ...
                        ${activeNavIndex === index 
                            ? 'text-white bg-[#5B3E31]/20 dark:text-white dark:bg-amber-400/20' 
                            : 'hover:text-[#ffffff]'
                        }
                    `}

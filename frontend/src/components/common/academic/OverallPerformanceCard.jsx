...
const OverallPerformanceCard = () => {
    ...
    useEffect(() => {
        const fetchDashboardSummary = async () => {
            ...
            try {
                setLoading(true);
                const response = await axios.get(`${API_BASE_URL}/api/academics/getDashboardSummary.php`, {
    ...
    if (loading || !user) {
        return (
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-lg h-[400px] animate-pulse">
                <div className="h-8 bg-gray-200 dark:bg-slate-700 rounded w-3/4 mb-8"></div>
    ...
    if (!performanceData) {
        return (
             <div className="bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl p-8 shadow-lg">
                <p>Could not load performance data at this time.</p>
            </div>
        );
    }
    ...
    return (
        <div className="bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl p-8 shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-gray-50 dark:bg-slate-700/50 p-6 rounded-2xl border border-gray-200 dark:border-slate-600">
                    <div className="flex items-center gap-2 mb-3">
                        <div className="p-2 bg-white dark:bg-slate-800 rounded-lg">
                            <TrendingUp className="w-5 h-5 text-green-600 dark:text-green-400" />
                        </div>
                        <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">General Average</span>
                    </div>
                    <div className="flex items-end gap-2 mb-3">
                        <p className={`text-5xl font-bold ${getGradeColorClass(performanceData.generalAverage)}`}>
                            {performanceData.generalAverage.toFixed(2)}
                        </p>
                        <p className="text-2xl font-semibold text-gray-400 dark:text-gray-500 mb-1">/100</p>
                    </div>
                     ...
                     <div className="flex items-center gap-4 mb-3">
                         <div className="relative w-20 h-20">
                             ...
                             <div className="absolute inset-0 flex items-center justify-center">
                                 <span className="text-xl font-bold text-gray-800 dark:text-gray-100">{performanceData.attendance}%</span>
                             </div>
                         </div>
                         <div>
                             <p className="text-2xl font-bold text-gray-800 dark:text-gray-100">{performanceData.totalDaysPresent}/{performanceData.totalDays}</p>
                ...
                <div className="bg-gray-50 dark:bg-slate-700/50 p-6 rounded-2xl border border-gray-200 dark:border-slate-600">
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">Class Participation</span>
                    </div>
                    <div className="flex items-center gap-1 mb-3">
                        {[...Array(5)].map((_, i) => (
                             <svg key={i} className={`w-8 h-8 ${i < performanceData.participation.rating ? 'text-amber-400' : 'text-gray-300 dark:text-slate-600'}`} fill="currentColor" viewBox="0 0 20 20">
